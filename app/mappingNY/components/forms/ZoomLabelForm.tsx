...
import { ZoomLabel as PrismaZoomLabel } from "@/prisma/generated_schema/myFirstDatabase";
...
const ZoomLabelForm = (props: ZoomLabelFormProps) => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            await fetch('/api/mappingNY/ZoomLabel')
            .then((response) => {
    ...
    const deleteLabel = async (label: PrismaZoomLabel) => {
        try {
            await fetch('/api/mappingNY/ZoomLabel/' + label.id, {
                method: 'DELETE',
                headers: {
        ...
        onSubmit: async (values) => {
            if(values.title?.length > 0) {
                ...
                if(selectedLabel == null) {
                    ...
                    try {
                        await fetch('/api/mappingNY/ZoomLabel/' + selectedLabel.id, {
                            method: 'PUT',
                            headers: {
                                'authorization': props.authToken,
                                'Content-Type': 'application/json',
