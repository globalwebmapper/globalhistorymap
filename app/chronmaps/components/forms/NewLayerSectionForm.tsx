...
import {LayerSection} from "@/prisma/generated_schema/chronmaps";
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            if(values.name?.length > 0) {
              if(submitType === "POST") {
                try {
                  await fetch('/api/chronmaps/LayerSection', {
                    method: 'POST',
                    headers: {
                ...
                if(props.layerSection) {
                  try {
                    await fetch('/api/chronmaps/LayerSection/' + props.layerSection.id, {
                      method: 'PUT',
                      headers: {
                        'authorization': props.authToken,
                        'Content-Type': 'application/json',
