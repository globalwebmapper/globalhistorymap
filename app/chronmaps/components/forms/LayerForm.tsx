...
import { LayerData as PrismaLayer } from "@/prisma/generated_schema/chronmaps";
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      */
      if (submitType === "POST") {
        try {
          const endpoint = props.standalone ? "/api/chronmaps/StandaloneLayers" : "/api/chronmaps/LayerData"
        ...
        if (props.layerConfig) {
          try {
            const response = await fetch("/api/chronmaps/LayerData/" + props.layerConfig.id, {
              method: "PUT",
              headers: {
                authorization: props.authToken,
                "Content-Type": "application/json",
        ...
        if (props.layerConfig) {
          try {
            await fetch("/api/chronmaps/LayerData/" + props.layerConfig.id, {
              method: "DELETE",
              headers: {
                authorization: props.authToken ?? "",
                "Content-Type": "application/json",
